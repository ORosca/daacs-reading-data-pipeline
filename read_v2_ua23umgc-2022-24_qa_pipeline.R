# read_v2_ua23umgc-2022-24_qa_pipeline.R
# ============================================================
# QA pipeline for UA23 + UMGC reading datasets (2022-2024)
#
# Purpose:
...
items_ua23_file <- file.path("D:/DData24/read.item.results.ua23.treat.rds")
items_umgc_file <- file.path("D:/DData24/read.item.results.umgc.treat.rds")

assess_ua23_file <- file.path("D:/DData24/read.assessments.ua23.rds")
assess_umgc_file <- file.path("D:/DData24/read.assessments.umgc.rds")

output_dir <- file.path(project_dir, "read_v2_ua23umgc-2022-24_qa_outputs")
...
# ============================================================
# Section 2C. Manual repair to unmatched QIDs
# ============================================================
...
# Clean up the repaired template_qid object
qid_lookup_obj$template_qid <- qid_lookup_obj$template_qid %>%
  mutate(
    domainId = dplyr::coalesce(domainId.x, domainId.y),
    stem = dplyr::coalesce(stem.x, stem.y)
  ) %>%
  select(
    source_name, assessment_id, block, question_id,
    domainId, difficulty, stem, question,
    stem_key, question_key,
    QID_original, QID, match_method, string_distance
  )
...
names(qid_lookup_obj$template_qid)
sum(is.na(qid_lookup_obj$template_qid$QID))
table(qid_lookup_obj$template_qid$match_method, useNA = "ifany")

safe_write_csv(
  qid_lookup_obj$match_diag_after_manual,
  file.path(output_dir, "qid_lookup_match_diag_after_manual.csv"),
  row.names = FALSE
)

safe_write_csv(
  qid_lookup_obj$template_qid %>%
    distinct(source_name, question_id, assessment_id, domainId, difficulty, stem, question,
             QID_original, QID, match_method, string_distance),
  file.path(output_dir, "template_qid_after_manual.csv"),
  row.names = FALSE
)

safe_write_csv(
  qid_lookup_obj$still_unmatched_after_manual %>%
    distinct(source_name, question_id, assessment_id, domainId, difficulty, stem, question,
             QID, match_method, string_distance),
  file.path(output_dir, "template_qid_still_unmatched_after_manual.csv"),
  row.names = FALSE
)

cat("\nManual repair applied.\n")
cat("Rows with manual QID repair: ",
    sum(qid_lookup_obj$template_qid$match_method == 'manual_repair', na.rm = TRUE), "\n")
cat("Still unmatched after manual repair: ",
    sum(is.na(qid_lookup_obj$template_qid$QID)), "\n")
...
cat("\nSaved reading 2022-24 outputs to:\n", output_dir, "\n")
cat("\nRows:\n")
