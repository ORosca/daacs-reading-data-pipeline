...
p_readTime_hist <- ggplot(read_v2_umgc_ua_22_23, aes(x = readTime)) +
  geom_histogram(bins = 30) +
  labs(title = "Read Time Distribution", x = "Read Time", y = "Count") +
  theme_minimal()

p_age_box <- ggplot(read_v2_umgc_ua_22_23, aes(y = age)) +
  geom_boxplot() +
  labs(title = "Boxplot of Age", y = "Age") +
  theme_minimal()

p_transfer_box <- ggplot(read_v2_umgc_ua_22_23, aes(y = transfer)) +
  geom_boxplot() +
  labs(title = "Boxplot of Transferred Credits", y = "Transferred Credits") +
  theme_minimal()

p_readTime_box <- ggplot(read_v2_umgc_ua_22_23, aes(y = readTime)) +
  geom_boxplot() +
...
writeLines(
  summary_line,
  con = file.path(output_dir, "read_v2_umgc_ua_22_23_item_count_quartiles.txt")
)
# ------------------------------------------------------------
...
write.csv(
  cat_descriptives_df,
  file.path(output_dir, "categorical_descriptives_read_v2_umgc_ua_22_23.csv"),
  row.names = FALSE
)

write.csv(
  num_descriptives,
  file.path(output_dir, "numeric_descriptives_read_v2_umgc_ua_22_23.csv"),
  row.names = FALSE
)
